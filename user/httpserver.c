...
ICACHE_FLASH_ATTR
int httpserver_parse_multipart_field_header(char *chunk, uint16_t chunklen) {
	char *temp = chunk;
	Request *req = &server->request;

	if ((temp = os_strstr(temp, "\r\n")) == NULL) {
		return MORE;  // More Data
	}
	
	if ((temp = os_strstr(temp, "Content-Disposition")) == NULL) {
		return MORE;  // More Data
	}

	// Field name
	if ((temp = os_strstr(temp + 2, "name=\"")) == NULL) {
		return MORE;  // More Data
	}
	temp += 6;
	...
	// Filename
	if ((temp = strstr(temp + 2, "filename=\"")) == NULL) {
		return MORE;  // More Data
	}
	temp += 10;
	
	req->multipartfilename = temp;
	if ((temp = strstr(temp, "\"")) == NULL) {
		return MORE;
	}
	temp[0] = '\0';
	...
	// Content-Type (only if exists)
	if ((temp = strstr(temp, "Content-Type: ")) != NULL) {
		temp += 14;
		req->multipart_contenttype = temp;
		if ((temp = strstr(temp, "\r\n")) == NULL) {
			return MORE;
		}
		temp[0] = '\0';
	...
	temp += 2;
	os_printf("Field: %s, %s, %s\r\n", req->multipartfield,
			req->multipartfilename, req->multipart_contenttype);
	return temp - chunk;
}


ICACHE_FLASH_ATTR
int httpserver_parse_multipart(Request *req, char *chunk, 
		uint16_t chunklen, uint16_t *bytesread, MultipartCallback callback) {
	int err;
	char *temp;
	char *field;
	char *nextfield;
	...
	*bytesread = 0;
	...
	while (true) {
		if (req->multipartfield == NULL) { 
			...
			if (err = httpserver_parse_multipart_field_header(
						temp, chunklen - (temp - chunk)) < 0) {
				os_printf("Cannot parse multipart field header: %d\r\n", err);
				if (err == MORE) {
					goto moredata;
				}
				return err;	
		...
		nextfield = strstr(field, req->boundary);
		if (nextfield != NULL) {
			fieldlen = nextfield - field;
		} 
		last = nextfield != NULL;
		
		callback(req->multipartfield, req->multipartfilename, field, fieldlen, 
				last);
		*bytesread = field - 
		if (last) {
			req->multipartfield = NULL;
			req->multipartfilename = NULL;
			req->multipart_contenttype = NULL;
		}
	}
